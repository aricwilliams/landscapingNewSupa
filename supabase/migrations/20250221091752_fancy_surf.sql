/*
  # Update channels and messages tables with proper RLS
...
DROP POLICY IF EXISTS "Allow read access to channels for all users" ON channels;
DROP POLICY IF EXISTS "Allow insert access to channels for all users" ON channels;
DROP POLICY IF EXISTS "Allow read access to messages for all users" ON messages;
DROP POLICY IF EXISTS "Allow insert access to messages for all users" ON messages;
...
BEGIN
  -- Drop existing policies if they exist
  DROP POLICY IF EXISTS "Allow public access to message images" ON storage.objects;
  DROP POLICY IF EXISTS "Allow upload access to message images for all users" ON storage.objects;
...
EXCEPTION
  WHEN others THEN
    ...
    RAISE NOTICE 'Error handling storage policies: %', SQLERRM;
END $$;